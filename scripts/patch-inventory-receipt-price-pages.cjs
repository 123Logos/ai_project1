...
const invTemplate =
  ...
  '<th data-v-4b5a159b>库房名称</th><th data-v-4b5a159b>品类名</th><th data-v-4b5a159b>库存</th><th data-v-4b5a159b>库存日期</th><th data-v-4b5a159b>操作</th>' +
...
async function loadInventoryList(){var e=document.getElementById(\`inv-table-body\`),t=document.getElementById(\`inv-count\`),n=document.getElementById(\`inv-page-size\`);if(n&&n.value&&(invPg.pageSize=parseInt(n.value,10)||invPg.pageSize),!e)return;E(e,5,\`正在加载库存数据...\`);try{var r=document.getElementById(\`inv-keyword\`),wh=document.getElementById(\`inv-wh-warehouse\`),cat=document.getElementById(\`inv-category-filter\`),needClientFilter=cat&&cat.value,i=new URLSearchParams;if(needClientFilter){i.set(\`page\`,\`1\`),i.set(\`page_size\`,\`9999\`)}else{i.set(\`page\`,String(invPg.page)),i.set(\`page_size\`,String(invPg.pageSize))}if(wh&&wh.value)i.set(\`warehouse_id\`,wh.value);else if(r&&r.value&&r.value.trim())i.set(\`keyword\`,r.value.trim());var a=z(await Api.request(\`GET\`,\`/tl/warehouse_inventories?\`+i.toString())),o=a.list||[];invPg.total=a.total||0;if(needClientFilter){var catId=String(cat.value);o=o.filter(function(t){return String(_(t,[\`品类id\`,\`category_id\`],\`\`))===catId});var total=o.length,start=(invPg.page-1)*invPg.pageSize;o=o.slice(start,start+invPg.pageSize);invPg.total=total}e.innerHTML=\`\`,o.length||D(e,5,\`暂无库存数据\`),o.forEach(function(t){var n=v(t,[\`id\`,\`record_id\`]),whId=_(t,[\`库房id\`,\`warehouse_id\`],\`\`),r=_(t,[\`库房名称\`,\`warehouse_name\`],\`-\`),catName=_(t,[\`品类名\`,\`品类\`,\`category_name\`,\`回收品种\`],\`-\`),i=Number(_(t,[\`当前库存\`,\`stock\`],null)),a=_(t,[\`库存日期\`,\`inventory_date\`],\`\`)||\`-\`,s=document.createElement(\`tr\`);s.innerHTML=\`<td>\`+M(r)+\`</td><td>\`+M(catName)+\`</td><td>\`+(isNaN(i)?\`-\`:i.toLocaleString())+\`</td><td>\`+M(a)+\`</td><td><button class="btn btn-sm btn-outline edit-inv-btn" data-id="\`+String(n)+\`" data-wh-id="\`+String(whId)+\`" data-stock="\`+String(isNaN(i)?0:i)+\`" data-date="\`+M(a)+\`">修改</button></td>\`,e.appendChild(s)}),t&&(t.textContent=String(invPg.total)),invUpdatePager()}catch(n){alert(\`加载库存失败: \`+(n.message||String(n))),t&&(t.textContent=\`0\`),e.innerHTML=\`\`,D(e,5,n.message||\`加载失败\`),invUpdatePager()}}
...
function initInventoryPage(){console.log(\`初始化当前库存管理页面\`);var e=document.getElementById(\`add-inv-btn\`);e&&e.addEventListener(\`click\`,function(){U(\`手工录入库存\`,invFormHtml(),{onOpen:async function(){await invFillWarehouseSelect(\`inv-warehouse\`);var e=document.getElementById(\`inv-date\`);e&&!e.value&&(e.value=kt())},onConfirm:function(){return invManualSubmit()}})});var t=document.getElementById(\`download-inv-template-btn\`);t&&t.addEventListener(\`click\`,function(){invDownloadTemplate()});var n=document.getElementById(\`import-inv-btn\`),r=document.getElementById(\`inv-import-input\`);n&&r&&(n.addEventListener(\`click\`,function(){r.click()}),r.addEventListener(\`change\`,function(e){invImportExcel(e&&e.target?e.target.files:null),r.value=\`\`}));var i=document.getElementById(\`inv-table-body\`);i&&i.addEventListener(\`click\`,function(e){var t=e.target.closest(\`.edit-inv-btn\`);if(t){var n=parseInt(t.getAttribute(\`data-wh-id\`),10)||parseInt(t.getAttribute(\`data-id\`),10),r=parseFloat(t.getAttribute(\`data-stock\`))||0,i=t.getAttribute(\`data-date\`)||\`\`;invEditStock(n,r,i)}});var a=document.getElementById(\`inv-search-btn\`);a&&a.addEventListener(\`click\`,function(){invPg.page=1,loadInventoryList()});var o=document.getElementById(\`inv-reset-btn\`);o&&o.addEventListener(\`click\`,function(){var e=document.getElementById(\`inv-keyword\`),wh=document.getElementById(\`inv-wh-warehouse\`),cat=document.getElementById(\`inv-category-filter\`);e&&(e.value=\`\`);wh&&(wh.value=\`\`);cat&&(cat.value=\`\`);invPg.page=1,loadInventoryList()});var s=document.getElementById(\`inv-prev-btn\`);s&&s.addEventListener(\`click\`,function(){invPg.page>1&&(--invPg.page,loadInventoryList())});var c=document.getElementById(\`inv-next-btn\`);c&&c.addEventListener(\`click\`,function(){invPg.page+=1,loadInventoryList()});var l=document.getElementById(\`inv-page-size\`);l&&l.addEventListener(\`change\`,function(){invPg.pageSize=parseInt(this.value,10)||20,invPg.page=1,loadInventoryList()});var u=document.getElementById(\`inv-keyword\`);u&&u.addEventListener(\`keydown\`,function(e){e.key===\`Enter\`&&(e.preventDefault(),invPg.page=1,loadInventoryList())});var whSearch=document.getElementById(\`inv-wh-search\`),whList=document.getElementById(\`inv-wh-list\`),whHidden=document.getElementById(\`inv-wh-warehouse\`);if(whSearch&&whList&&whHidden){var whData=[];async function invLoadWhData(){if(!whData.length){try{var e=await Api.request(\`GET\`,\`/tl/get_warehouses\`),r=Api.unwrapList(e).filter(function(e){return!b(T(e))});whData=r.map(function(e){return{id:w(e),name:T(e)||\`库房 #\`+w(e)}})}catch(e){}}}function invRenderWhList(filter){var v=(filter||\`\`).trim().toLowerCase();var items=whData.filter(function(e){return!v||e.name.toLowerCase().includes(v)});whList.innerHTML=items.length?items.map(function(e){return\`<button type="button" class="wh-sel-opt\`+(String(e.id)===whHidden.value? \` wh-sel-opt--on\`: \`\`)+\`" data-wh-id="\`+String(e.id)+\`">\`+M(e.name)+\`</button>\`}).join(\`\`):\`<p class="text-muted" style="margin:8px;text-align:center;font-size:13px;">无匹配库房</p>\`;whList.style.display=\`block\`;whList.querySelectorAll(\`button[data-wh-id]\`).forEach(function(btn){btn.addEventListener(\`click\`,function(){var id=btn.getAttribute(\`data-wh-id\`);var item=whData.find(function(e){return String(e.id)===id});whSearch.value=item?item.name:\`\`;whHidden.value=id||\`\`;whList.style.display=\`none\`})})}whSearch.addEventListener(\`focus\`,async function(){await invLoadWhData();invRenderWhList(whSearch.value)});whSearch.addEventListener(\`input\`,function(){invRenderWhList(this.value)});document.addEventListener(\`click\`,function(e){if(!whSearch.contains(e.target)&&!whList.contains(e.target))whList.style.display=\`none\`})}var catSelect=document.getElementById(\`inv-category-filter\`);if(catSelect){Api.request(\`GET\`,\`/tl/get_category_mapping\`).then(function(e){var r=V(Api.unwrapData(e)||{}).list,items=ue(se(Array.isArray(r)?r:[]));catSelect.innerHTML=\`<option value="">全部品种</option>\`+items.map(function(e){var t=e.id,n=String(e.name||\`\`).trim();return t!=null&&!isNaN(Number(t))&&n? \`<option value="\`+String(t)+\`">\`+M(n)+\`</option>\`: \`\`}).join(\`\`)}).catch(function(){})}loadInventoryList()}
...
async function loadReceiptPriceList(){var e=document.getElementById(\`rcp-table-body\`),t=document.getElementById(\`rcp-count\`),n=document.getElementById(\`rcp-page-size\`);if(n&&n.value&&(rcpPg.pageSize=parseInt(n.value,10)||rcpPg.pageSize),!e)return;E(e,5,\`正在加载收货价格...\`);try{var r=document.getElementById(\`rcp-keyword\`),wh=document.getElementById(\`rcp-warehouse-filter\`),cat=document.getElementById(\`rcp-category-filter\`),i=new URLSearchParams;i.set(\`page\`,String(rcpPg.page)),i.set(\`page_size\`,String(rcpPg.pageSize));if(wh&&wh.value)i.set(\`warehouse_id\`,wh.value);else if(r&&r.value&&r.value.trim())i.set(\`keyword\`,r.value.trim());if(cat&&cat.value)i.set(\`category_id\`,cat.value);var a=z(await Api.request(\`GET\`,\`/tl/warehouse_receipt_prices?\`+i.toString())),o=a.list||[];rcpPg.total=a.total||0,e.innerHTML=\`\`,o.length||D(e,5,\`暂无收货价格数据\`),o.forEach(function(t){var n=v(t,[\`id\`,\`price_id\`]),r=_(t,[\`库房名称\`,\`warehouse_name\`],\`-\`),i=_(t,[\`品类名\`,\`品类名称\`,\`category_name\`],\`-\`),a=Number(_(t,[\`价格\`,\`price\`],null)),d=_(t,[\`价格日期\`,\`price_date\`,\`日期\`],\`-\`),o=document.createElement(\`tr\`);o.innerHTML=\`<td>\`+M(r)+\`</td><td>\`+M(i)+\`</td><td>\`+(isNaN(a)?\`-\`:a.toLocaleString())+\`</td><td>\`+M(d)+\`</td><td><button class="btn btn-sm btn-outline edit-btn" data-id="\`+String(n)+\`">编辑</button> <button class="btn btn-sm btn-danger delete-btn" data-id="\`+String(n)+\`">删除</button></td>\`,e.appendChild(o)}),t&&(t.textContent=String(rcpPg.total)),rcpUpdatePager()}catch(n){alert(\`加载收货价格失败: \`+(n.message||String(n))),t&&(t.textContent=\`0\`),e.innerHTML=\`\`,D(e,5,n.message||\`加载失败\`),rcpUpdatePager()}}
...
function initReceiptPricePage(){console.log(\`初始化收货价格管理页面\`);var e=document.getElementById(\`add-rcp-btn\`);e&&e.addEventListener(\`click\`,function(){U(\`手工录入收货价格\`,rcpFormHtml(),{onOpen:async function(){await invFillWarehouseSelect(\`rcp-warehouse\`),await invFillCategorySelect(\`rcp-category\`)},onConfirm:function(){return rcpManualSubmit()}})});var t=document.getElementById(\`download-rcp-template-btn\`);t&&t.addEventListener(\`click\`,function(){rcpDownloadTemplate()});var n=document.getElementById(\`import-rcp-btn\`),r=document.getElementById(\`rcp-import-input\`);n&&r&&(n.addEventListener(\`click\`,function(){r.click()}),r.addEventListener(\`change\`,function(e){rcpImportExcel(e&&e.target?e.target.files:null),r.value=\`\`}));var i=document.getElementById(\`rcp-table-body\`);i&&i.addEventListener(\`click\`,function(e){var t=e.target.closest(\`.edit-btn\`),n=e.target.closest(\`.delete-btn\`);if(t){var r=parseInt(t.getAttribute(\`data-id\`),10),i=t.closest(\`tr\`),a=i&&i.children&&i.children[3]?parseFloat(String(i.children[2].textContent||\`\`).replace(/,/g,\`\`)):NaN;rcpEditPrice(r,a)}n&&rcpDeletePrice(parseInt(n.getAttribute(\`data-id\`),10))});var a=document.getElementById(\`rcp-search-btn\`);a&&a.addEventListener(\`click\`,function(){rcpPg.page=1,loadReceiptPriceList()});var o=document.getElementById(\`rcp-reset-btn\`);o&&o.addEventListener(\`click\`,function(){var e=document.getElementById(\`rcp-keyword\`),wh=document.getElementById(\`rcp-warehouse-filter\`),cat=document.getElementById(\`rcp-category-filter\`);e&&(e.value=\`\`);wh&&(wh.value=\`\`);cat&&(cat.value=\`\`);rcpPg.page=1,loadReceiptPriceList()});var s=document.getElementById(\`rcp-prev-btn\`);s&&s.addEventListener(\`click\`,function(){rcpPg.page>1&&(--rcpPg.page,loadReceiptPriceList())});var c=document.getElementById(\`rcp-next-btn\`);c&&c.addEventListener(\`click\`,function(){rcpPg.page+=1,loadReceiptPriceList()});var l=document.getElementById(\`rcp-page-size\`);l&&l.addEventListener(\`change\`,function(){rcpPg.pageSize=parseInt(this.value,10)||20,rcpPg.page=1,loadReceiptPriceList()});var u=document.getElementById(\`rcp-keyword\`);u&&u.addEventListener(\`keydown\`,function(e){e.key===\`Enter\`&&(e.preventDefault(),rcpPg.page=1,loadReceiptPriceList())});var whSearch=document.getElementById(\`rcp-wh-search\`),whList=document.getElementById(\`rcp-wh-list\`),whHidden=document.getElementById(\`rcp-warehouse-filter\`);if(whSearch&&whList&&whHidden){var whData=[];async function rcpLoadWhData(){if(!whData.length){try{var e=await Api.request(\`GET\`,\`/tl/get_warehouses\`),r=Api.unwrapList(e).filter(function(e){return!b(T(e))});whData=r.map(function(e){return{id:w(e),name:T(e)||\`库房 #\`+w(e)}})}catch(e){}}}function rcpRenderWhList(filter){var v=(filter||\`\`).trim().toLowerCase();var items=whData.filter(function(e){return!v||e.name.toLowerCase().includes(v)});whList.innerHTML=items.length?items.map(function(e){return\`<button type="button" class="wh-sel-opt\`+(String(e.id)===whHidden.value? \` wh-sel-opt--on\`: \`\`)+\`" data-wh-id="\`+String(e.id)+\`">\`+M(e.name)+\`</button>\`}).join(\`\`):\`<p class="text-muted" style="margin:8px;text-align:center;font-size:13px;">无匹配库房</p>\`;whList.style.display=\`block\`;whList.querySelectorAll(\`button[data-wh-id]\`).forEach(function(btn){btn.addEventListener(\`click\`,function(){var id=btn.getAttribute(\`data-wh-id\`);var item=whData.find(function(e){return String(e.id)===id});whSearch.value=item?item.name:\`\`;whHidden.value=id||\`\`;whList.style.display=\`none\`})})}whSearch.addEventListener(\`focus\`,async function(){await rcpLoadWhData();rcpRenderWhList(whSearch.value)});whSearch.addEventListener(\`input\`,function(){rcpRenderWhList(this.value)});document.addEventListener(\`click\`,function(e){if(!whSearch.contains(e.target)&&!whList.contains(e.target))whList.style.display=\`none\`})}var catSelect=document.getElementById(\`rcp-category-filter\`);if(catSelect){Api.request(\`GET\`,\`/tl/get_category_mapping\`).then(function(e){var r=V(Api.unwrapData(e)||{}).list,items=ue(se(Array.isArray(r)?r:[]));catSelect.innerHTML=\`<option value="">全部品种</option>\`+items.map(function(e){var t=e.id,n=String(e.name||\`\`).trim();return t!=null&&!isNaN(Number(t))&&n? \`<option value="\`+String(t)+\`">\`+M(n)+\`</option>\`: \`\`}).join(\`\`)}).catch(function(){})}loadReceiptPriceList()}
`
...
// 删除旧的函数（如果存在）
// 策略：找到 logicAnchor 之前的所有注入函数，全部移除
// 注入函数可能以 rcpUpdatePager 开头（早于 invFormHtml），所以向前搜索
if (app.includes('function invFormHtml()') && app.includes(logicAnchor)) {
  const anchorIdx = app.indexOf(logicAnchor)
  const invFnStart = app.indexOf('function invFormHtml()')
  if (invFnStart !== -1 && invFnStart < anchorIdx) {
    // 向前搜索注入块的起始位置（可能在 invFormHtml 之前）
    let blockStart = invFnStart
    const prevFns = ['function invUpdatePager()', 'function rcpUpdatePager()', 'function invFormHtml()']
    for (const fn of prevFns) {
      const pos = app.lastIndexOf(fn, invFnStart)
      if (pos !== -1 && pos < blockStart) {
        blockStart = pos
      }
    }
    // 从 blockStart 到 logicAnchor 之间的所有内容都是旧的注入函数
    app = app.substring(0, blockStart) + app.substring(anchorIdx)
  }
}
